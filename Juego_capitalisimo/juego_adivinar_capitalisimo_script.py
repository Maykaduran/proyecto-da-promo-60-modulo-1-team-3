...
# Datos iniciales (países y capitales)
...
def jugar_capitalisimo():
    input("Presiona Enter para empezar el juego...")  # Pausa inicial
    ...
    print(f"{'=' * 50} \n\nSI ESTÁS LEYENDO ESTO, HAS SIDO SELECCIONADA PARA UNA OPORTUNIDAD ÚNICA GRACIAS A NUESTRO PATROCINADOR __________, SOLO SABRÁS CUÁL ES AL FINAL DE ESTE JUEGO:\nComienzas con {puntuacion} puntos. El máximo de rondas es: {max_rondas} rondas.\n")
    print("=" * 50)

    usuario = input('¿Te ves preparada? ¿Lo has traído todo? (SI/NO): ').strip().lower()
    if usuario != 'si':
        try:
            from pyfiglet import figlet_format
            ahbueno = figlet_format("AH BUENO,\nSE ME CUIDA", font='bell')
            print(ahbueno)
        except:
            print(""""
,,     .    __  __      ____   .     . .____  __    _   ___    
 //    /|    |   |       /   \  /     / /      |\   |  .'   `.  
      /  \   |___|       |,_-<  |     | |__.   | \  |  |     |  
     /---'\  |   |       |    ` |     | |      |  \ |  |     |  
   ,'      \ /   /       `----'  `._.'  /----/ |   \|   `.__.' ,
                                                               /
   _____ .____       __   __ .____         ___  .     . _ .___       .   
  (      /           |    |  /           .'   \ /     / | /   `     /|   
   `--.  |__.        |\  /|  |__.        |      |     | | |    |   /  \  
      |  |           | \/ |  |           |      |     | | |    |  /---'\ 
 \___.'  /----/      /    /  /----/       `.__,  `._.'  / /---/ ,'      \"""")
                  
        input("Presiona Enter para salir...")  # Pausa antes de salir
        return
    ...
    while ronda_actual <= max_rondas and puntuacion > 0:  # Corregido: ahora verifica puntuacion > 0
        ...
        input("Presiona Enter para continuar...")  # Pausa después de cada ronda
        ronda_actual += 1

    print("\n" + "=" * 50)
    print(f"Hasta aquí nuestro cuestionario\nTu puntuación final es: {puntuacion}")
    if puntuacion >= 8:
        print("ENHORABUENA: Has ganado con {puntuacion} puntos\n")
        print("¡HAS SIDO ELEGIDA PRESIDENTE DEL MUNDO! ¡ESPERAMOS MUCHO DE TI!\nMUCHA SUERTE...")
    elif puntuacion <= 3:
        print(f"Has perdido con {puntuacion} puntos.\nEl mundo no es lo tuyo, aunque perder también tiene recompensa:\n")
        print("¡HAS SIDO ELEGIDA PARA SER EXPULSADA DEL PLANETA!\n\nESTAMOS PREPARÁNDOTE UNA BURRA PARA QUE NO TE FALTE DE NADA AL ABANDONAR LA TIERRA.\nPronto recibirás la fecha.")
        try:
            print(figlet_format("Recuerda revisar\ntu carpeta de spam", font='bell'))
        except:
            print("Recuerda revisar tu carpeta de spam.")
    else:
        print(f'Ni has perdido ni has ganado, tienes {puntuacion} puntos.\nLa mediocridad no es mala: al final son solo capitales')

    input("\nPresiona Enter para salir...")  # Pausa final
